...
Run all commands from the **repository root** (`recall`). Script paths are relative to root: `.\scripts\...`
...
- **Exit 1:** One or more tools missing; the script prints **winget** and **choco** install commands.

Optional:

- **`-Fix`** — When LLVM is installed but `LIBCLANG_PATH` is not set, sets `LIBCLANG_PATH` for the current user to `C:\Program Files\LLVM\bin`. Does not run winget/choco.
...
| Check + auto-set LIBCLANG_PATH | `.\scripts\check-screenpipe-prereqs.ps1 -Fix` |
